...
from app.db_models_assistant import Person, Phones, Address, Files, Note, Tag, GoogleFiles, create_db
import itertools
...
@app.route("/note-information", methods=["GET", "POST"], strict_slashes=False)
@login_required
def index_note():
    notes1 = session.query(Note).all()
    return render_template("tags_templates/note-information.html", notes=notes1)
...
def add_note():
    ...
    return render_template("tags_templates/note.html", tags=tags)
...
def add_tag():
    ...
    return render_template("tags_templates/tag.html")
...
def note_edit(note_id):
    ...
    if request.method == "POST":
        ...
        return render_template("tags_templates/edit-note.html", tags=tags1)
...
def finding_notes():
    if request.method == "POST":
        ...
        if note_list and not tag_list:
            return render_template("tags_templates/find-note.html", info_note=note_list)
        if tag_list and not note_list:
            return render_template("tags_templates/find-note.html", info_tag=tag_list)
        if finding_note and not tag_list:
            return render_template(
                "tags_templates/find-note.html",
        ...
        if tag_list and note_list:
            return render_template(
                "tags_templates/find-note.html", info_note=note_list, info_tag=tag_list
            )
        if not finding_tag_name and not finding_note:
            return render_template("tags_templates/find-note.html")
    if request.method == "GET":
        return render_template("tags_templates/find-note.html")
